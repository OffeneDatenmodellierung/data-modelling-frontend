...
### 5. Optional: Helper Conversion Function

Consider adding a helper function to convert from simplified product formats to ODPS-compliant format:

```rust
impl ODPSExporter {
    /// Convert a simplified product structure to ODPS-compliant format
    /// This helps frontend applications that use simplified data structures
    pub fn convert_to_odps_format(
        simplified: &SimplifiedProduct
    ) -> Result<ODPSDataProduct, ExportError> {
        // Convert simplified format to ODPS format
...
- Frontend currently implements workaround conversion function in `odpsService.ts`
