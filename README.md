...
- **Decision Logs** (SDK 1.13.4+): MADR-format Architecture Decision Records with status workflow
- **Knowledge Base** (SDK 1.13.4+): Documentation articles with types (Guide, Tutorial, Reference, etc.)
...
The application uses a WASM build of the `data-modelling-sdk` (version **1.13.4**) for offline functionality:

1. **SDK Version**: Requires `data-modelling-sdk = "1.13.4"` crate
...
**Note**: The SDK must be version 1.13.4 or compatible. The API project (`data-modelling-api`) is available on [crates.io](https://crates.io/crates/data-modelling-api) and uses `data-modelling-sdk = "1.13.4"` with features `["api-backend", "git"]`.
