...
export const DomainCanvas: React.FC<DomainCanvasProps> = ({ workspaceId, domainId }) => {
  ...
  const { onNodeClick: onTableNodeClick, onNodeDragStop, onEdgeClick, onConnect } = useCanvas(workspaceId, domainId);
  
  // State for table metadata modal
  const [selectedTableForMetadata, setSelectedTableForMetadata] = React.useState<typeof tables[0] | null>(null);
  ...
  const handleTableCardClick = React.useCallback((table: typeof tables[0]) => {
  ...
  // Handle table edit
  const handleTableEdit = React.useCallback((tableId: string) => {
    setSelectedTable(tableId);
    // The ModelEditor will handle opening the table editor when selectedTableId changes
  }, [setSelectedTable]);

  // Handle table delete
  const handleTableDelete = React.useCallback((tableId: string) => {
  ...
  // Handle table export
  const handleTableExport = React.useCallback(async (tableId: string) => {
    const table = tables.find((t) => t.id === tableId);
    if (!table) return;
  ...
  const handleSystemDelete = React.useCallback((systemId: string) => {
    const system = systems.find((s) => s.id === systemId);
    if (!system) return;

    if (confirm(`Are you sure you want to delete the system "${system.name}"? This will remove all associated tables and assets from this system.`)) {
  ...
  const handleAssetDelete = React.useCallback((assetId: string) => {
  ...
  const handleAssetExport = React.useCallback(async (assetId: string) => {
    const asset = computeAssets.find((a) => a.id === assetId);
    if (!asset) return;
  ...
  const handleTableBPMNClick = React.useCallback((tableId: string) => {
    // Find BPMN process that has transformation links involving this table
    const linkedProcess = bpmnProcesses.find((p) => 
      p.domain_id === domainId && 
      p.transformation_links?.some((link) => 
        link.source_table_id === tableId || link.target_table_id === tableId
      )
    );
  ...
  const handleAssetBPMNClick = React.useCallback((assetId: string) => {
    const asset = computeAssets.find((a) => a.id === assetId);
  ...
  const handleAssetDMNClick = React.useCallback((assetId: string) => {
    const asset = computeAssets.find((a) => a.id === assetId);
  ...
  const tableHasBPMN = React.useCallback((tableId: string): boolean => {
    return bpmnProcesses.some((p) => 
      p.domain_id === domainId && 
      p.transformation_links?.some((link) => 
        link.source_table_id === tableId || link.target_table_id === tableId
      )
  ...
  const visibleTables = useMemo(() => {
    ...
    if (selectedSystemId && (currentView === 'process' || currentView === 'operational' || currentView === 'analytical')) {
      const selectedSystem = systems.find((s) => s.id === selectedSystemId);
      console.log(`[DomainCanvas] Filtering by selected system ${selectedSystemId}:`, {
        system: selectedSystem ? { id: selectedSystem.id, name: selectedSystem.name, table_ids: selectedSystem.table_ids } : null,
    ...
    return filtered;
  }, [getFilteredTables, currentView, selectedDataLevel, domainId, selectedSystemId, systems, tables]);
  ...
  const initialNodes: Node[] = useMemo(() => {
    ...
    if (currentView === 'systems') {
      domainSystems.forEach((system, index) => {
        ...
        const systemTables = visibleTables.filter((table) => 
          system.table_ids?.includes(table.id)
        );
    ...
    const computeAssetNodes = currentView === 'process' 
  ...
  }, [domainSystems, visibleTables, domainComputeAssets, selectedTableId, selectedSystemId, domainId, currentView, domains, handleTableCardClick, handleTableEdit, handleTableDelete, handleTableExport, handleSystemEdit, handleSystemDelete, handleAssetEdit, handleAssetDelete, handleAssetExport]);
  ...
  const initialEdges: Edge[] = useMemo(() => {
    const relationshipEdges = domainRelationships
      ...
      .map((relationship) => {
        ...
        const sourceType = relationship.source_type || (relationship.source_table_id ? 'table' : 'table');
        const targetType = relationship.target_type || (relationship.target_table_id ? 'table' : 'table');
        ...
        const useCardinalityEdge = isTableToTable && (currentView === 'operational' || currentView === 'analytical');
  ...
  useEffect(() => {
    ...
    const currentTableDataHash = visibleTables.map((t) => 
    ...
    <div className="w-full h-full relative" data-testid="domain-canvas">
      ...
      {(currentView === 'process' || currentView === 'operational' || currentView === 'analytical') && (
        <>
          ...
          {currentView === 'process' && (
            <NodeViewActions domainId={domainId} />
          )}
        </>
      )}
