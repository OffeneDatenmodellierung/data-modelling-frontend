    ...
    for (const otherEdge of allEdges) {
      ...
      for (const otherSeg of otherSegments) {
        ...
        if (
          ...
          intersection.x !== undefined &&
          intersection.y !== undefined &&
  ...
  return (
    <>
      ...
      <defs>
        ...
        >
          <path d="M0,0 L7,4 L0,8 Z" fill={selected ? '#3b82f6' : '#6b7280'} />
        </marker>
      </defs>

      {/* Edge path with arrow */}
      <BaseEdge id={id} path={finalPath} style={edgeStyle} markerEnd={`url(#${markerId})`} />
